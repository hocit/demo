jQuery.fn.extend({
	treetable: function() {
		...
		items.forEach(function (item) {
			...
			var left = parent.left - item.left + 45;
		...
		$table.on("click", "div.tt div.content", function (e) {
			...
			if (item.showChildren === true) {
				...
				hide(id);
				$(e.currentTarget).closest(".tt").find(".arrow-1").show();
				$(e.currentTarget).closest(".tt").find(".arrow-2").hide();
			}
			else {
				...
				$(e.currentTarget).closest(".tt").find(".arrow-1").hide();
				$(e.currentTarget).closest(".tt").find(".arrow-2").show();
			}
		});
